...
로컬 개발 시 `400: invalid_request` 오류가 발생하거나 리다이렉트 주소가 `0.0.0.0` 등으로 나타나면 다음을 조치하세요:
1. **`.env.local` 파일 생성**: 프로젝트 루트에 파일을 만들고 아래 내용을 추가합니다:
   - `AUTH_URL=http://localhost:3000` (반드시 명시)
   - `NEXTAUTH_URL=http://localhost:3000`
2. **구글 클라우드 콘솔 설정**:
   - **사용자 유형**: **'외부(External)'** (Gmail 계정 기준 필수)
   - **테스트 사용자**: 본인 이메일 직접 등록.
   - **승인된 리디렉션 URI**: `http://localhost:3000/api/auth/callback/google` 정확히 포함.
3. **서버 재시작**: 환경 변수 적용을 위해 `npm run dev`를 껐다가 다시 켭니다.
...
4. **🚨 에이전트 실수 우선 점검 원칙 (최우선)**: 오류가 발생했을 때, CSP·캐시·환경·외부 라이브러리 등 외부 요인을 핑계로 삼기 전에, **에이전트 본인이 직접 작성하거나 수정한 코드 실수(오타, 로직 오류, 미배포, import 누락 등)를 0순위로 먼저 점검**할 것. 외부 요인은 자체 실수 가능성을 완전히 배제한 뒤 마지막으로 검토한다.
5. **🛰️ 깃허브 푸시(Push) 의무 보고**: 코드 수정 및 시스템 업데이트 작업이 완료되면, 반드시 **깃허브 푸시 여부를 사용자에게 명시적으로 보고**하여 온라인 배포 상태를 실시간으로 확인할 수 있게 할 것.
