...
### 🔄 루틴 사이클 시스템 & 데이터 안정화 (Stable Sync)
- **30일 사이클 시스템 안내**: 30일 루틴이 종료되면 자동으로 다음 명화와 함께 새로운 빈 캔버스가 시작되는 시스템 설계 보완.
- **샘플 데이터 로드(Guest)**: 비로그인 사용자 및 신규 사용자가 서비스 컨셉을 한눈에 볼 수 있도록 `/setup` 페이지에 샘플 데이터 로드 버튼 상시 공개.
- **자동 초기 데이터 생성**: 비로그인 상태로 진입 시 데이터가 전혀 없는 경우, 자동으로 샘플 데이터를 생성하여 튜토리얼 역할을 수행하도록 개선.
- **동기화 로직 보정**: 서버-로컬 간 동기화 시 비어있는 서버 데이터가 로컬 기록을 덮어씌우지 않도록 우선순위 재정립.
- **버전 관리**: `v0.3.0` 버전 정보를 콘솔에 출력하고 내부 정합성 점검 완료.
